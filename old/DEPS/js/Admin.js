/*
    ...
    notes: 11/30/2021 - Refactored entire app. Removed unwanted features (Functional Areas & Verions). 
                        Streamlined to not use as many ajax calls to copy questions.  Added copy all functionality.
...
Admin = {
    ...
    checklistTemplateListName: "Checklist",
    programsListName: "Programs",
    siteDirectoryListName: "Directory",
    commandInspectionChecklistsListName: "Command Inspection Checklists",
    ...
    */
    ...
    docReady: function(){
	    ...
	    this.path = _spPageContextInfo.serverRequestPath;
        ...
        getUserAssignedGroups: function(){
			...
			$.ajax({
				...
				$.each(data.d.results, function(i,val){
					switch (val.Id) {
						case Admin.adminGroup.id:
							Admin.isOwner = true;
                            Admin.isInspector = true;
							break;
						case Admin.inspectorGroup.id:
							Admin.isInspector = true;
							break;
						default:
             ...
             //   console.log(data.d.results);
                ...
                $("#kendoProgramDDL").kendoDropDownList({
                    ...
                    close: function(e){
                        ...
                        Admin.selectedProgramTitle = this._oldText;
            ...
            $.ajax({
                ...
                $("#kendoE3CommandDDL").kendoDropDownList({
					...
					change: function (e) {
                        ...
                        Admin.destSiteTitle = this._oldText;
        ...
        getExistingEch3Program: function(){
            ...
            $.ajax({
                ...
                if(data.d.results.length > 0){
                    ...
                    kendo.confirm("This program already exists on "+Admin.destSiteTitle+".<br/><br/>" + "This action will DELETE all existing questions and add these as new ones.  This action cannot be un-done.<br/><br/>" +
    ...
    grid: {
        configureChecklistGrid: function() {
            ...
            $("#checklistGrid").kendoGrid({
                    ...
                    transport: {
                        create: {      
                            ...
                            complete: function (jqXHR, textStatus) {
                                if(textStatus == 'error') {
                                    kendo.alert("Error creating new question(s). Refresh the screen or check your data and try again. Contact IT Support if this problem persists.<br/><br/>" +
                        ...
                        update: {
                            ...
                            complete: function (jqXHR, textStatus) {
                                if(textStatus == 'error') {
                                    kendo.alert("Error updating question(s). Refresh the screen or check your data and try again. Contact IT Support if this problem persists.");
				...
				resizable: true,
				filterable: true,
                toolbar: [{"name": "create", "attr": "style=float:left"},
                ...
                dataBound: function(){
                    ...
                    if(Admin.isOwner)
                        Admin.utilities.enableGridButtons();
                },
				edit: function(){
                    Admin.grid.checkForChanges();
                },
    ...
    copy: {
            ...
            setTimeout(function() { 
                ...
                context.executeQueryAsync(
                    ...
                    Function.createDelegate(this, function() {
                        Admin.utilities.hideSpinner();
                        kendo.alert("Successfully copyied the questions to the new program!");
                        Admin.copy.toggleCopySection();
                    }),
                    //on failure
                    Function.createDelegate(this, function(sender, args) {
                        Admin.utilities.hideSpinner();
                        kendo.alert("Error copying selected checklist questions.  Refresh the screen and try again or contact IT Support if the problem persists.<br/><br/>" + args.get_message());
                    })
        ...
        sendToE3Checklist: function() {
            ...
            setTimeout(function() { 
                ...
                context.executeQueryAsync(
                    Function.createDelegate(this, function() { //success
                        Admin.utilities.hideSpinner();
                    ...
                    Function.createDelegate(this, function(sender, args) { //failure
                        Admin.utilities.hideSpinner();
        ...
        confirmCopyAll: function(){
            kendo.confirm("This will copy EVERY valid question (not Obsolete) to the selected ECH III command site.<br/><br/>It will NOT replace existing questions.<br/><br/>" + 
        ...
        allQuestions: function(){
            ...
            $.ajax({
                ...
                var context = new SP.ClientContext(Admin.destSiteURL),
                    web = context.get_web(),
                    list = web.get_lists().getByTitle(Admin.commandInspectionChecklistsListName),
                    itemsCopied = 0;
                ...
                for (var k=0; k < numberOfBatches; k++) {
                    ...
                    for (var p=0; p < batchSize; p++){
                        ...
                        if(itemsCopied < questionCount){
                            ...
                            listItem.set_item('Reference', questions[itemsCopied].Reference);
                            listItem.set_item('ReferenceData', questions[itemsCopied].ReferenceData);
                            listItem.set_item('ReferenceDate', questions[itemsCopied].ReferenceDate);
                            listItem.set_item('QuestionNumber', questions[itemsCopied].QuestionNumber);
                    ...
                    context.executeQueryAsync(
                        ...
                        Function.createDelegate(this, function() { //success
                            ...
                            if(batchesCommitted == numberOfBatches){
                                Admin.utilities.hideSpinner();
                                kendo.alert("Success creating "+itemsCopied+" items on the Ech III Command Self-Assessment Checklist at this URL: " + Admin.destSiteURL);
                            }
                            console.log("batch copied=" + batchesCommitted + " of " + numberOfBatches);
                        }),
                        Function.createDelegate(this, function(sender, args) { //failure
                            Admin.utilities.hideSpinner();
                            kendo.alert("Error sending all items to Ech III site on batch# " + batchesCommitted + " of " + numberOfBatches + " batches.<br/><br/>Contact IT Support or purge the destination list, fix the error and try again.<br/><br/>" + args.get_message());
                            console.log("Error sending batch to Ech III site for batch#: " + batchesCommitted + " of " + numberOfBatches + "\n" + "\n" + args.get_message());
                ...
                Admin.utilities.hideSpinner();
                kendo.alert("Error retrieving the questions from the checklist template. Refresh the screen and try again, or contact IT Support if this problem persists.<br/><br/>" + JSON.stringify(error));
                console.log("Error retrieving the questions from the checklist template. ERROR: " + JSON.stringify(error));
                console.log(error);
            });
        }
    },
    ...
    utilities: {
        ...
        prompt: function(){            
            kendo.prompt("Enter the name of the new Program")
                .done(function(data){
        ...
        enableSendButtons: function(val){
            if(val){
                ...
                $('#submitSendToInspectionChecklistButton').attr("disabled", 'disabled');
        ...
        deleteEch3Programs: function(){
            ...
            var caml = "<View><Query><Where><Eq><FieldRef Name='Program'/><Value Type='Text'>"+Admin.selectedProgramTitle+"</Value></Eq></Where></Query></View>";
            var context = new SP.ClientContext(Admin.destSiteURL);
            var web = context.get_web();
            var list = web.get_lists().getByTitle(Admin.commandInspectionChecklistsListName);
            ...
            context.executeQueryAsync(
                Function.createDelegate(this, function(){  
                    ...
                    context.executeQueryAsync(
                        ...
                        Function.createDelegate(this, function(sender, args){
                            //failed on delete
                            Admin.utilities.hideSpinner();
                            kendo.alert("Error deleting existing questions from the Ech III checklist.<br/><br/>" + "stack: "+ args.get_message());
                ...
                Function.createDelegate(this, function(sender, args){
                    ...
                    Admin.utilities.hideSpinner();
                    kendo.alert("Error loading existing questions from the Ech III checklist.<br/><br/>" + "stack: "+ args.get_message());
        ...
        confirmBatchDelete: function(){
            kendo.confirm("This will delete EVERY valid question on the following ECH III command site:<br/><br/>"+Admin.destSiteTitle+"<br/><br/>" + 
                ...
                Function.createDelegate(this, function() { 
                    ...
                    for(var i=0; i<numberOfBatches; i++){
                        ...
                        context.executeQueryAsync(
                            ...
                            Function.createDelegate(this, function(sender, args){
                                Admin.utilities.hideSpinner();
                                kendo.alert("Errors deleting questions from Ech III checklist.<br/><br/>" + "stack: "+ args.get_message() + "<br/><br/>If the server timed out, just click again to delete another batch.");
...
SP.SOD.executeFunc('sp.js', 'SP.ClientContext', Admin.docReady());
