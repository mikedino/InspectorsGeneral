...
/**
 * Generates CSV data from the status report items
 */
export class Export {
    ...
    private static generate(items: Array<any>) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            for (let j = 0; j < this._fields.length; j++) {
                ...
                // see if this is a rich text and strip any html
    ...
    static generateCSV(items: Array<any> = []) {
        ...
        if (Blob && navigator && navigator["msSaveBlob"]) {
            // Download the file
            navigator["msSaveBlob"](new Blob([csv], { type: "data:text/csv;charset=utf-8;" }), "isr_data.csv");
        } else {
            // Generate an anchor
            var anchor = document.createElement("a");
            anchor.download = "isr_data.csv";
    ...
    static init(): PromiseLike<void> {
        ...
        return new Promise((resolve, reject) => {
            ...
            // Load the custom fields from the Item content type
            List(Strings.Lists.ISR).ContentTypes().query({ Top: 1, Expand: ["Fields"] }).execute(ct => {
